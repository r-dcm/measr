...
alpha_patt <- alpha_patt |>
  rowwise() |>
  mutate(
    prob = profile_prob(c_across(everything()), prob = eta),
    .before = 1
  ) |>
  ungroup()
...
## dina and dino
dinao_params <- tibble(
  item = glue("item_{seq_len(num_item)}"),
...
dina_data <- stu_xi_dina |>
  ...
  mutate(
    prob_correct = ((1 - slip)^high_prob) * (guess^(1 - high_prob)),
...
dina_stan <- list(
  ...
  alpha = as.matrix(select(alpha_patt, -prob)),
  xi = dina_xi
)
dina_mod <- stan(
  here("data-raw", "stan", "no-strc-dinao.stan"),
  data = dina_stan,
  chains = 4,
  cores = 4,
  iter = 500
